      ...
      {totalMonthly > 0 && (
        <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 10, padding: 16, marginBottom: 14 }}>
          ...
          <div style={{ display: 'flex', height: 22, borderRadius: 6, overflow: 'hidden' }}>
            {fixedCosts > 0 && <div style={{ width: `${(fixedCosts/totalMonthly)*100}%`, background: T.cyan, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, color: T.bg, fontWeight: 600 }}>Fixed {Math.round((fixedCosts/totalMonthly)*100)}%</div>}
            {variableCosts > 0 && <div style={{ width: `${(variableCosts/totalMonthly)*100}%`, background: T.amber, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, color: T.bg, fontWeight: 600 }}>Variable {Math.round((variableCosts/totalMonthly)*100)}%</div>}
          </div>
        </div>
      )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
          {modules.map(m => (
            <div key={m.name} style={{
              ...
              <div style={{ fontSize: 10, color: m.active ? T.cyan : T.textDim, fontWeight: 600 }}>{m.name}</div>
              <div style={{ fontSize: 13, fontWeight: 700, color: m.active ? T.text : T.textDim, fontFamily: "'JetBrains Mono', monospace", marginTop: 3 }}>
